...
using namespace std;
control_block cb;
std::vector<int> anx, any;

// External functions
double *alloc1D(int m,int n);
void cmdLine(int argc, char *argv[]);
void init (double *E,double *E_prev,double *R,int m,int n);
void initMPI(double *E_prev, double *R, control_block cb, int m, int n, int bx, int by);
...
// nx x ny blocks
int fnx = cb.m / cb.px, fny = cb.n / cb.py;
int lastrank = nprocs - 1, lastx = lastrank / cb.py, lasty = lastrank % cb.py;
int lastnx = cb.m - (fnx * lastx), lastny = cb.n - (fny * lasty);
std::vector<int> allnx(cb.px, fnx), allny(cb.py, fny);
allnx[cb.px-1] = lastnx;
allny[cb.py-1] = lastny;
if (lastnx > fnx + 1) {
  int cnt = lastnx - fnx, cur = 0;
  for (; cnt > 0; --cnt) {
    allnx[cur++] += 1;
    allnx[cb.px - 1] -= 1;
  }
...
any = allny;
 ...
 if (myrank == 0) {
    ...
    int bx = allnx[0], by = allny[0];
    for (int rank=1; rank < nprocs; ++rank) {
        int rx = rank / cb.py, ry = rank % cb.py;
        int nx = allnx[rx], ny = allny[ry];
        // if (rx + 1 == cb.px && cb.m % cb.px) nx = cb.m - (nx * rx);
        // if (ry + 1 == cb.py && cb.n % cb.py) ny = cb.n - (ny * ry);

        // allocate for rank process
        E_prev = alloc1D(nx+2, ny+2);
        R = alloc1D(nx+2, ny+2);

        // init according to pos & send to corresponding process
        // printf("====================================RANK %d====================================\n", rank);
        // printf("NX: %d NY: %d RX: %d RY: %d\n", nx, ny, rx, ry);
        initMPI(E_prev,R,cb,nx,ny,bx,by);
        bx += nx;
        by += ny;
        // printMat("E_prev", E_prev, nx, ny);
        // printMat("R", R, nx, ny);

        MPI_Send(E_prev, (nx+2)*(ny+2), MPI_DOUBLE, rank, rank, MPI_COMM_WORLD);
        MPI_Send(R, (nx+2)*(ny+2), MPI_DOUBLE, rank, rank, MPI_COMM_WORLD);
        free(E_prev);
        free(R);
    }
    int nx = allnx[0], ny = allny[0];
    E = alloc1D(nx+2, ny+2);
    E_prev = alloc1D(nx+2, ny+2);
    R = alloc1D(nx+2, ny+2);
    initMPI(E_prev,R,cb,nx,ny,0,0);
   //  printf("====================================RANK %d====================================\n", myrank);
   //  printMat("E_prev", E_prev, fnx, fny);
   //  printMat("R", R, fnx, fny);
 } else {
    int rx = myrank / cb.py, ry = myrank % cb.py;
    int nx = allnx[rx], ny = allny[ry];
    // if (rx + 1 == cb.px && cb.m % cb.px) nx = cb.m - (nx * rx);
    // if (ry + 1 == cb.py && cb.n % cb.py) ny = cb.n - (ny * ry);
    E = alloc1D(nx+2, ny+2);
    ...
    MPI_Status eprev, r;
    printf("Im here%d\n", myrank);
    MPI_Recv(E_prev, (nx+2)*(ny+2), MPI_DOUBLE, 0, myrank, MPI_COMM_WORLD, &eprev);
    MPI_Recv(R, (nx+2)*(ny+2), MPI_DOUBLE, 0, myrank, MPI_COMM_WORLD, &r);
    printf("====================================RANK %d by MPI====================================\n", myrank);
   //  printf("NX: %d NY: %d RX: %d RY: %d\n", nx, ny, rx, ry);
   //  printMat("E_prev", E_prev, nx, ny);
   //  printMat("R", R, nx, ny);
 }
 ...
 printf("I am about to begin!!!! %d", myrank);
 // End Initialization
